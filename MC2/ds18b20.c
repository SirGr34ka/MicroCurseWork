...
void ds18b20_PortInit(void)
{
  ...
  GPIOB->CRH |= GPIO_CRH_MODE12;			// Output Mode, max speed 50MHz, GPIOB->CRH |=   11000000000000b, MODE12 = 11
  GPIOB->CRH |= GPIO_CRH_CNF12_0;			// General purpose output Open-drain, GPIOB->CRH |=  100000000000000b, CNF12 = 01
  GPIOB->CRH &= ~GPIO_CRH_CNF12_1;		// General purpose output Open-drain, GPIOB->CRH &= 0111111111111111b, CNF12 = 01
}
//--------------------------------------------------
uint8_t ds18b20_Reset(void)
{
  uint16_t status;
	GPIOB->BSRR = GPIO_BSRR_BR12;					// set 0 to PIN12
  DelayMicro(480);											// delay 480 mcs
  GPIOB->BSRR = GPIO_BSRR_BS12;					// set 1 to PIN12
  DelayMicro(60);												// delay 60 mcs
  status = GPIOB->IDR & GPIO_IDR_IDR12;	// check value on PIN12, if not zero then device not responds
  DelayMicro(480);											// delay 480 mcs
  return (status ? 1 : 0);
}
//----------------------------------------------------------
uint8_t ds18b20_ReadBit(void)
{
  ...
  DelayMicro(1);															 // delay 1 mcs
	...
	bit = (GPIOB->IDR & GPIO_IDR_IDR12 ? 1 : 0); // check value on PIN12
